...
def CornerDetection():
    ...
    for file in all_image_files:
        ...
        find, corners = cv2.findChessboardCorners(img, (8, 11), None)
        if find:
            criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
            corners = cv2.cornerSubPix(img, corners, (5, 5), (-1, -1), criteria)
            img_corner = img.copy()
            cv2.drawChessboardCorners(img_corner, (8, 11), corners, find)
...
class StereoDisparityMap(QtWidgets.QFrame):
    ...
    def Stereo_map(self):
        ...
        # self.disparity_visual = ((disparity_map - min_disp) / (max_disp - min_disp) * 255).astype(np.uint8)
        self.disparity_visual = cv2.normalize(self.disparity_map, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    ...
    def check_disparity_value(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            print(f"dissparity value: {disparity_value}")
            ...
            if disparity_value != 0:
                ...
                cv2.circle(image_R_copy, (int(right_x), y), 20, (0, 255, 0), 5)
...
class VGG19(QtWidgets.QFrame):
    ...
    def Inference(self):
        ...
        inference_transforms = transforms.Compose(
            [transforms.Resize(32), transforms.ToTensor(), transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))]
        )
...
def show_augmentedimg():
    augmented_img = []
    Dtransforms = transforms.Compose([transforms.RandomVerticalFlip(), transforms.RandomHorizontalFlip(), transforms.RandomRotation(30)])
    for i in PIL_imgs:
